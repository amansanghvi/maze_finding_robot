...
boolean Adafruit_VL6180X::begin(TwoWire *theWire, uint8_t i2caddr) {
  _i2caddr = VL6180X_DEFAULT_I2C_ADDR; //Set address to default to allow the new adress to be written
  ...
  _i2c-> begin();
  if(i2caddr != VL6180X_DEFAULT_I2C_ADDR){
    changeAddress(i2caddr); //change addres from default to new address
  }
